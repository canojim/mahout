...
public class ParallelALSFactorizationJob extends AbstractJob {
	...
	static final String ALPHA = ParallelALSFactorizationJob.class.getName()
			+ ".alpha";
	static final String NUM_ENTITIES = ParallelALSFactorizationJob.class
			.getName() + ".numEntities";

	static final String USES_LONG_IDS = ParallelALSFactorizationJob.class
			.getName() + ".usesLongIDs";
	static final String TOKEN_POS = ParallelALSFactorizationJob.class.getName()
			+ ".tokenPos";

	static final String PATH_TO_YTY = ParallelALSFactorizationJob.class
			.getName() + ".pathToYty";;
	...
	public int run(String[] args) throws Exception {
		...
		/* create an initial M */
		initializeM(averageRatings, numItems);
	 ...
	 * @throws IOException
	 */
	private void initializeM(Vector averageRatings, int numEntity)
			throws IOException {
		...
		try {	
				...
				for (Vector.Element e : averageRatings.nonZeroes()) {
					...
					if (partId != prevPartId) {
						...
						String partPath = "part-m-" + String.format("%05d", partId);
	...
	private void runSolver(Path ratings, Path output, Path pathToUorM,
			...
			int numEntities) throws ClassNotFoundException, IOException,
			InterruptedException {
